...
class Spider(Spider):
    ...
    def extract_middle_text(self, text, start_str, end_str, pl, start_index1: str = '', end_index2: str = ''):
                ...
                for i in range(len(plx)):
                    ...
                    for match in matches:
                        ...
                        if 'http' not in match[0]:
                            output += f"#{'📽️尚爱👉' + match[1]}${number}{xurl}{match[0]}"
                        else:
                            output += f"#{'📽️尚爱👉' + match[1]}${number}{match[0]}"
                    output = output[1:]
                    purl = purl + output + "$$$"
                purl = purl[:-3]
                return purl
        ...
        if pl == 2:
            ...
            matches = re.findall(start_index1, middle_text)
            if matches:
                new_list = [f'✨尚爱👉{item}' for item in matches]
                jg = '$$$'.join(new_list)
                return jg

    def homeContent(self, filter):
        result = {}
        result = {"class": [{"type_id": "27", "type_name": "尚爱短剧🌠"}],
                  "list": [],
                  "filters": {
                      "4": [{"key": "年代",
                             "name": "年代",
    ...
    def detailContent(self, ids):
        ...
        content = '白云飘过🎉为您介绍剧情📢本资源来源于网络🚓侵权请联系删除👉' + self.extract_middle_text(res,'<meta name="description" content=','/>', 0)
